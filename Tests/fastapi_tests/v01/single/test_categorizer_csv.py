def test_process_categorize_csv(
    client, encoded_csv, perform_post_request, validate_encoded_response
):
    """
    ...
    payload = {
        "file_encoded": encoded_csv,
        "extension": ".csv",
        "index_column": "index",  # Example value for index_column
        ...
        "categories": [
            ...
            {"name": "negative", "description": "Negative Category"},  # Another example category
        ],
        "mode": "Evaluation",  # Use 'Evaluation' or 'Production' as Mode
        "model": ["Zero Shot", "Few Shot"],  # Example model list
